(function () {
    ...
    document.addEventListener('click', async (e) => {
        ...
        switch (action) {
            case 'edit-alliance':
                ...
                break;
            case 'invite-team':
                openInviteModal(el.dataset.id);
                break;
            case 'delete-alliance':
                await handleDeleteAlliance(el.dataset.id);
                break;
            case 'import-data':
                await openImportModal(el.dataset.id);
                break;
            case 'leave-alliance':
                await handleRemoveMember(el.dataset.id, parseInt(el.dataset.team));
                break;
            case 'accept-invite':
                await handleRespondInvite(el.dataset.id, true);
                break;
            case 'decline-invite':
                await handleRespondInvite(el.dataset.id, false);
                break;
        }
    });
    ...
    document.addEventListener('change', async (e) => {
        ...
        if (!el) return;
        const allianceId = el.dataset.allianceId;
    ...
    async function handleImportSubmit() {
        ...
        const allianceId = document.getElementById('import-alliance-id').value;
